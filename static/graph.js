...
function drawGraph(graphData) {
    try {
        ...
        // Append a group element to center the graph elements within the SVG
        ...
        svg.call(zoom);
        svg.call(zoom.transform, d3.zoomIdentity.scale(0.9)); // Initial zoom level
        ...
        // Force Simulation with explicit centering at the middle of the container
        const simulation = d3.forceSimulation(nodes)
            .force("link", d3.forceLink(links).id(d => d.id).distance(150))
            .force("charge", d3.forceManyBody().strength(-400))
            .force("center", d3.forceCenter(width / 2, height / 2)) // Center based on container dimensions
